(function () {
	...
	Eye.prototype.draw = function (ctx) {
		...
		var pupilX = this.pos[0] - this.radius * (2/3) * cosine * ((this.pos[0] - this.watched.pos[0])/ Math.max(this.pos[0], Asteroids.Game.DIM_X - this.pos[0])),
			pupilY = this.pos[1] - this.radius * (2/3) * sine * ((this.pos[1] - this.watched.pos[1])/Math.max(this.pos[1], Asteroids.Game.DIM_Y - this.pos[1]));
		ctx.arc(
			pupilX,
			pupilY,
		...
		//Draws the gleam of the ship in the eye!
		if (this.watched.draw != undefined) {
			this.watched.draw(ctx, [pupilX, pupilY], (1 - pupilFactor) * 0.6);
		}
	}
})();